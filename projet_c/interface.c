...
/*cree une fenetre gtk*/
{
	...
	ptWindow = gtk_window_new(GTK_WINDOW_TOPLEVEL);
	gtk_window_set_position(GTK_WINDOW(ptWindow), GTK_WIN_POS_CENTER );
	gtk_window_set_default_size(GTK_WINDOW(ptWindow),400,500);
	gtk_window_set_title(GTK_WINDOW(ptWindow),"Instructions");
	return ptWindow;
}
...
void afficherInstructions(GtkWidget *ptWindow)
{
	...
	ptBox = gtk_vbox_new(TRUE,0);
	ptLabel = gtk_label_new("Bienvenue sur Bubble Epic !\n\n\nEclatez toutes les bulles pour gagner.\nLe score est calculé en fonction de la couleur de la bulle éclatée et de la difficulté.\nATTENTION !!!\nLa technique de l'essuie-glace est totalement interdite sous peine de sanction immédiate.\nAmusez vous bien.\n\n© Copyright 2016 TD.JC");
	gtk_label_set_justify(GTK_LABEL(ptLabel), GTK_JUSTIFY_CENTER);
	gtk_container_add(GTK_CONTAINER(ptWindow),ptBox);
	gtk_box_pack_start(GTK_BOX(ptBox),ptLabel,TRUE, FALSE, 0);
...
/*creation d'une table qui met en place tout les elements sur une grille*/
{
	...
	gtk_table_attach( GTK_TABLE(ptTable), ptTexte,0,6,0,1,GTK_EXPAND,GTK_EXPAND,0,0);
	gtk_table_attach( GTK_TABLE(ptTable), ptBoutonS,7,9,0,1,GTK_EXPAND | GTK_FILL,GTK_EXPAND,0,0);
...
/*reaffiche les boutons de difficulte apres que la partie soit finie afin d'en refaire une autre*/
{
